...
public:
    ...
    uint256 TransactionHash() const
    {
        if (components.size())
        {
            ...
            else if (components[0].elType == CTransactionHeader::TX_SOLANA_OBJECT && components[0].Rehydrate(vdxfObj))
            {
                CDataStream s = CDataStream(vdxfObj.data, SER_NETWORK, PROTOCOL_VERSION);
                uint256 prevtxid;
                CSolanaCrossChainExport scce;
                ...
                {
                    LogPrintf("Deserialization of SOLANA type object failed : %s\n", e.what());
                    return uint256();
                }
