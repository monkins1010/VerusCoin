...
CONFIG_SITE="$PWD/depends/$HOST/share/config.site" ./configure "$HARDENING_ARG" "$LCOV_ARG" "$TEST_ARG" "$MINING_ARG" "$PROTON_ARG" $CONFIGURE_FLAGS CPPFLAGS='-g -mabi=lp64 -march=armv8-a' CXXFLAGS='-g -mabi=lp64 -march=armv8-a' --with-arch=aarch64 --with-CPU=arm 
"$MAKE" "$@" V=1
